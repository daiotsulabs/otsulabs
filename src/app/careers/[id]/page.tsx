...
import { Center, Container, Heading, useDisclosure, Text, Flex, Box, Button, useMediaQuery } from '@chakra-ui/react'
...
export default function JobId() {
  ...
  return (
    <Layout>
      ...
      {
        job ? <>
          <Container maxWidth={'600'} className='h-full animate-page-fade pt-[80px] md:pt-[124px] px-4 md:px-0' color='#707070'>
            <Heading
            ...
            <Text className='text-[8px] md:text-[12px] uppercase tracking-[0.6px] leading-[normal] mb-6 md:mb-8' color={'#CCC'}>{job?.time}</Text>
                  ...
                  <ul className='list-disc pl-6'>
                    {
                      job?.responsibilities.map((r, index) => (
                        <li key={index}>
                          <Text className='text-[12px] md:text-base 2xl:text-xl leading-[normal]'>{r}</Text>
                        </li>
                      ))
                    }
                  </ul>
            ...
            <Center
              ...
              <Box width={'100%'} className='mx-auto'>
                ...
                {
                  isMobileScreen &&
                  <Box onClick={goBack} className={`${mcQueenDisplay.className} uppercase text-xs text-center mt-5 mb-3 cursor-pointer color-white`} color={'#ffffff'} letterSpacing={1.2}>
                    Back to careers
                  </Box>
                }
              </Box>
